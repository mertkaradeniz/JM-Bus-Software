...
{
    public partial class frmTopluEpostaGonderme : Form
    {
        public frmTopluEpostaGonderme()
        {
            InitializeComponent();
        }

        SqlConnection baglan = new SqlConnection(ClsDatabase.data_base());
        ClsMail MailGonder = new ClsMail();
        ...
        private void frmTopluEpostaGonderme_Load(object sender, EventArgs e)
        {
            ...
            comboBox1.Text = (Decrypt(gln[0]).Split('-')[2].Split('+')[0]);   
        ...
        private void arac()
        {
            ...
            SqlCommand command = new SqlCommand("select distinct eposta from Yolcular", this.baglan);
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < listView2.CheckedItems.Count; i++)
            {
                ...
                if (!MailGonder.MAILGONDER(comboBox1.Text, listView2.CheckedItems[i].SubItems[1].Text, textBox1.Text.Trim()))
                    BASARISIZSMS += 1;
                else
                    BASARILISMS += 1;
